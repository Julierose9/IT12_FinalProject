...
<aside class="sidebar">
              ...
              <div class="filter-menu" id="filterMenu" style="display: none;">
                ...
                <div class="filter-section">
                  ...
                  <div class="filter-options">
                    ...
                    <div class="filter-option">
                      <input type="checkbox" id="status-suspended" checked>
                ...
                <!-- User Role Filters -->
                <div class="filter-section">
                  <div class="filter-section-title">User Roles</div>
                  <div class="filter-options">
                    <div class="filter-option">
                ...
                <!-- Date Created Filter -->
                <div class="filter-section">
                  <div class="filter-section-title">Date Created</div>
                  <div class="filter-options">
                    <div class="filter-option">
                      <input type="radio" name="dateCreated" id="date-all" checked>
                      <label for="date-all">All Time</label>
                    </div>
                    <div class="filter-option">
                      <input type="radio" name="dateCreated" id="date-week">
                      <label for="date-week">This Week</label>
                    </div>
                    <div class="filter-option">
                      <input type="radio" name="dateCreated" id="date-month">
                      <label for="date-month">This Month</label>
                    </div>
                    <div class="filter-option">
                      <input type="radio" name="dateCreated" id="date-custom">
  ...
  <script>
    ...
    document.querySelectorAll('input[name="dateCreated"]').forEach(radio => {
    ...
    document.getElementById('applyFilters').addEventListener('click', function() {
      ...
      // Here you would typically refresh account data based on filters
      console.log('Filters applied - refreshing account data...');
    });
    ...
    function updateCurrentFilters() {
      ...
      if (document.getElementById('status-active').checked) {
        currentFilters.accountStatus.push('Active');
      }
      if (document.getElementById('status-inactive').checked) {
        currentFilters.accountStatus.push('Inactive');
      }
      if (document.getElementById('status-suspended').checked) {
        currentFilters.accountStatus.push('Suspended');
      }
      
      // Update user roles
      currentFilters.userRoles = [];
      if (document.getElementById('role-all').checked) {
        currentFilters.userRoles.push('All Roles');
      } else {
        if (document.getElementById('role-admin').checked) {
          currentFilters.userRoles.push('Administrator');
        }
        if (document.getElementById('role-cashier').checked) {
          currentFilters.userRoles.push('Cashier');
        }
        if (document.getElementById('role-manager').checked) {
          currentFilters.userRoles.push('Manager');
        }
      }
      
      // Update date created
      if (document.getElementById('date-all').checked) {
        currentFilters.dateCreated = 'All Time';
      } else if (document.getElementById('date-week').checked) {
        currentFilters.dateCreated = 'This Week';
      } else if (document.getElementById('date-month').checked) {
        currentFilters.dateCreated = 'This Month';
      } else if (document.getElementById('date-custom').checked) {
        const fromDate = document.getElementById('dateFrom').value;
        const toDate = document.getElementById('dateTo').value;
        currentFilters.dateCreated = `Custom: ${fromDate} to ${toDate}`;
      }
    }
    ...
    function updateActiveFilters() {
      ...
      const hasCustomFilters = 
        currentFilters.accountStatus.length < 3 ||
        currentFilters.userRoles.length !== 1 || 
        currentFilters.userRoles[0] !== 'All Roles' ||
        currentFilters.dateCreated !== 'All Time';
      ...
      // Add account status filter tags if not all are selected
      if (currentFilters.accountStatus.length < 3) {
        currentFilters.accountStatus.forEach(status => {
          const statusTag = createFilterTag(`Status: ${status}`, `status-${status.toLowerCase()}`);
          activeFilters.appendChild(statusTag);
        });
      }
      
      // Add user role filter tags if not "All Roles"
      if (currentFilters.userRoles.length > 0 && 
          (currentFilters.userRoles.length > 1 || currentFilters.userRoles[0] !== 'All Roles')) {
        currentFilters.userRoles.forEach(role => {
          const roleTag = createFilterTag(`Role: ${role}`, `role-${role.toLowerCase()}`);
          activeFilters.appendChild(roleTag);
        });
      }
      
      // Add date created filter tag if not default
      if (currentFilters.dateCreated !== 'All Time') {
        const dateTag = createFilterTag(`Date: ${currentFilters.dateCreated}`, 'dateCreated');
    ...
    function removeFilter(filterType) {
      ...
      if (filterType.startsWith('status-')) {
        ...
        const index = currentFilters.accountStatus.indexOf(
          filterName.charAt(0).toUpperCase() + filterName.slice(1)
        );
        if (index > -1) {
          currentFilters.accountStatus.splice(index, 1);
        }
      } else if (filterType.startsWith('role-')) {
        const filterName = filterType.replace('role-', '');
        const index = currentFilters.userRoles.indexOf(
          filterName.charAt(0).toUpperCase() + filterName.slice(1)
        );
        if (index > -1) {
          currentFilters.userRoles.splice(index, 1);
        }
      ...
      // Here you would typically refresh account data
      console.log('Filter removed - refreshing account data...');
    }
    
    function updateFilterInputs() {
      // Update account status checkboxes
      document.getElementById('status-active').checked = currentFilters.accountStatus.includes('Active');
      document.getElementById('status-inactive').checked = currentFilters.accountStatus.includes('Inactive');
      document.getElementById('status-suspended').checked = currentFilters.accountStatus.includes('Suspended');
      
      // Update user role checkboxes
      document.getElementById('role-all').checked = currentFilters.userRoles.includes('All Roles');
      document.getElementById('role-admin').checked = currentFilters.userRoles.includes('Administrator');
      document.getElementById('role-cashier').checked = currentFilters.userRoles.includes('Cashier');
      document.getElementById('role-manager').checked = currentFilters.userRoles.includes('Manager');
      
      // Update date created radio
      if (currentFilters.dateCreated === 'All Time') {
        document.getElementById('date-all').checked = true;
      } else if (currentFilters.dateCreated === 'This Week') {
        document.getElementById('date-week').checked = true;
      } else if (currentFilters.dateCreated === 'This Month') {
